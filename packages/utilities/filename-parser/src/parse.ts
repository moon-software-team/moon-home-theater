...
export const filenameParse = (
  ...
  const languagesMatches = matchManyExpressions(withoutTitle, LANGUAGES_EXPS)?.map(
    (result) => result.value
  );
  ...
  return filterEmpty({
    ...
    providers: matchManyExpressions(filename, PROVIDERS_EXPS)?.map((result) =>
      groupToProvider(result.groups as { [key: string]: string })
    ),
    title: parsed.title,
    year: parsed.year,
